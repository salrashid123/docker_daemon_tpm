...
func ReadCertsDirectory(tlsConfig *tls.Config, directory string) error {
	...
	for _, f := range fs {
		...
		if strings.HasSuffix(f.Name(), ".cert") {
			...
			tpmKeyName := certName[:len(certName)-5] + ".bin"
			logrus.Debugf("cert: %s", filepath.Join(directory, f.Name()))
			if !hasFile(fs, keyName) && !hasFile(fs, tpmKeyName) {
				return fmt.Errorf("missing key %s for client certificate %s. Note that CA certificates should use the extension .crt", keyName, certName)
			}
			if !hasFile(fs, tpmKeyName) {
		...
		if strings.HasSuffix(f.Name(), ".bin") {
			...
			logrus.Printf("Loading TPM key: %s", filepath.Join(directory, f.Name()))

			keyName := f.Name()
			certName := keyName[:len(keyName)-4] + ".cert"

			if !hasFile(fs, "ca.crt") {
				return fmt.Errorf("Missing CA for %s for key %s", certName, keyName)
			}
			if !hasFile(fs, certName) {
				return fmt.Errorf("Missing client certificate %s for key %s", certName, keyName)
			}
